...
def build_story_nodes():
    ...
    for char_id, data in storylines.items():
        nodes[f'intro_{char_id}'] = {
            'text': data['intro'],
            'choices': [
                {'text': c[0], 'statChange': c[1], 'next': f"lunch_{char_id}"} for c in data['intro_choices']
            ]
        }
        nodes[f'lunch_{char_id}'] = {
            'text': data['lunch'],
            'choices': [
                {'text': c[0], 'statChange': c[1], 'next': f"prep_{char_id}"} for c in data['lunch_choices']
            ]
        }
        nodes[f'prep_{char_id}'] = {
            'text': data['prep'],
            'choices': [
                {'text': c[0], 'statChange': c[1], 'next': f"rumor_{char_id}"} for c in data['prep_choices']
            ]
        }
        nodes[f'rumor_{char_id}'] = {
            'text': data['rumor'],
            'choices': [
                {'text': c[0], 'statChange': c[1], 'next': f"festival_{char_id}"} for c in data['rumor_choices']
            ]
        }
        nodes[f'festival_{char_id}'] = {
            'text': data['festival'],
            'choices': [
                {'text': c[0], 'statChange': c[1], 'next': f"{c[2]}_{char_id}"} for c in data['festival_choices']
            ]
        }
        nodes[f'investigate_{char_id}'] = {
            'text': data['investigate'],
            'choices': [
                {'text': c[0], 'statChange': c[1], 'next': f"aftermath_{char_id}"} for c in data['investigate_choices']
            ]
        }
        nodes[f'follow_{char_id}'] = {
            'text': data['follow'],
            'choices': [
                {'text': c[0], 'statChange': c[1], 'next': f"aftermath_{char_id}"} for c in data['follow_choices']
            ]
        }
        nodes[f'daily_{char_id}'] = {
            'text': data['daily'],
            'choices': [
                {'text': c[0], 'statChange': c[1], 'next': f"aftermath_{char_id}"} for c in data['daily_choices']
            ]
        }
        nodes[f'aftermath_{char_id}'] = {
            'text': data['aftermath'],
            'choices': [
