...
CHARACTERS = {
    'm1': {
        'name': "洛頁彥",
        'gender': 'male',
        'image': '/static/images/characters/m1.png',
        'appearance': "笑起來有些孩子氣，艷紅色的頭髮，情緒都寫在臉上",
        'personality': "性格開朗、大膽，敢於主動探尋未知事物。有時會因為粗心 or 錯估自己能力而出錯，但都能積極補救局面。",
        'traits': ["開朗", "大膽", "粗心", "積極"],
        'quote': "「本大爺帶著你衝過去，怕什麼！」"
    },
    'm2': {
        'name': "齊勻楠",
        'gender': 'male',
        'image': '/static/images/characters/m2.png',
    ...
    'm3': {
        'name': "秦陌寂",
        'gender': 'male',
        'image': '/static/images/characters/m3.png',
    ...
    'f1': {
        'name': "田媛寧",
        'gender': 'female',
        'image': '/static/images/characters/f1.png',
    ...
    'f2': {
        'name': "張栖鈴",
        'gender': 'female',
        'image': '/static/images/characters/f2.png',
    ...
    'f3': {
        'name': "顧音棉",
        'gender': 'female',
        'image': '/static/images/characters/f3.png',
...
def build_story_nodes():
    nodes = {
        ...
        'select_target_m': {
            ...
            'choices': [
                ...
                {'text': "隨機決定", 'targetKey': "random", 'next': "intro_random"}
            ]
        },
        'select_target_f': {
            'text': "請選擇你想攻略的對象：",
    ...
    for char_id, data in storylines.items():
        nodes[f'intro_{char_id}'] = {
            'text': data['intro'],
            'choices': [
                {'text': c[0], 'statChange': c[1], 'next': f"lunch_{char_id}_{'a' if idx == 0 else ('b' if idx == 1 else 'c')}"} for idx, c in enumerate(data['intro_choices'])
            ]
        }
        
        for route in ['a', 'b', 'c']:
            nodes[f'lunch_{char_id}_{route}'] = {
                'text': data[f'lunch_{route}'],
                'choices': [
                    {'text': c[0], 'statChange': c[1], 'next': f"prep_{char_id}_{'a' if idx == 0 else ('b' if idx == 1 else 'c')}"} for idx, c in enumerate(data[f'lunch_{route}_choices'])
                ]
            }
            nodes[f'prep_{char_id}_{route}'] = {
                'text': data[f'prep_{route}'],
                'choices': [
                    {'text': c[0], 'statChange': c[1], 'next': f"rumor_{char_id}_{'a' if idx == 0 else ('b' if idx == 1 else 'c')}"} for idx, c in enumerate(data[f'prep_{route}_choices'])
                ]
            }
            nodes[f'rumor_{char_id}_{route}'] = {
                'text': data[f'rumor_{route}'],
                'choices': [
                    {'text': c[0], 'statChange': c[1], 'next': f"festival_{char_id}_{'a' if idx == 0 else ('b' if idx == 1 else 'c')}"} for idx, c in enumerate(data[f'rumor_{route}_choices'])
                ]
            }
            nodes[f'festival_{char_id}_{route}'] = {
                'text': data[f'festival_{route}'],
                'choices': [
                    {'text': c[0], 'statChange': c[1], 'next': f"{c[2]}_{char_id}"} for c in data[f'festival_{route}_choices']
                ]
            }

        nodes[f'investigate_{char_id}'] = {
            'text': data['investigate'],
            'choices': [
                {'text': c[0], 'statChange': c[1], 'next': f"aftermath_{char_id}_b"} for c in data['investigate_choices']
            ]
        }
        nodes[f'follow_{char_id}'] = {
            'text': data['follow'],
            'choices': [
                {'text': c[0], 'statChange': c[1], 'next': f"aftermath_{char_id}_a"} for c in data['follow_choices']
            ]
        }
        nodes[f'daily_{char_id}'] = {
            'text': data['daily'],
            'choices': [
                {'text': c[0], 'statChange': c[1], 'next': f"aftermath_{char_id}_c"} for c in data['daily_choices']
            ]
        }
        
        for route in ['a', 'b', 'c']:
            nodes[f'aftermath_{char_id}_{route}'] = {
                'text': data[f'aftermath_{route}'],
                'choices': [
                    {'text': c[0], 'statChange': c[1], 'next': "eval_chapter3"} for c in data[f'aftermath_{route}_choices']
                ]
            }
...
def parse_text(text, target_key):
    return text
